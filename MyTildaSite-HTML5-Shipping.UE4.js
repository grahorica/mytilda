...
var Module = {
	...
	UE4_indexedDBName: 'UE4_assetDatabase_MyTildaSite', // this should be an ascii ID string without special characters that is unique to the project that is being packaged
	UE4_indexedDBVersion: 202404212001, // Bump this number to invalidate existing IDB storages in browsers.
};
...
Module.arguments = ['../../../MyTildaSite/MyTildaSite.uproject','-stdout',];
...
//	}
	...
	function withIndexedDB(db) {
		...
		var mainCompiledCode = fetchFromIndexedDB(db, 'wasmModule').then(function(wasmModule) {
			...
			return download(Module.locateFile('https://dl.dropboxusercontent.com/scl/fi/o89r5a9pig6xf2lrisi9v/MyTildaSite-HTML5-Shipping.wasm?rlkey=76smvy7om5c6yaw3hnjo9ckgs&st=xr62b697&dl=0'), 'arraybuffer').then(function(wasmBytes) {
		...
		// ----------------------------------------
		// MAIN JS
		var mainJsDownload = fetchOrDownloadAndStore(db, Module.locateFile('https://rawcdn.githack.com/grahorica/mytilda/bc891c40a1bf41677932a7482cf3897934bde8b6/MyTildaSite-HTML5-Shipping.js'), 'blob').then(function(data) {
		...
		// ----------------------------------------
		// MORE JS
		var dataJsDownload = fetchOrDownloadAndStore(db, Module.locateFile('https://rawcdn.githack.com/grahorica/mytilda/10948055b490b6ec93f65dd94c83df63b067f73a/MyTildaSite-HTML5-Shipping.data.js'));
		var utilityJsDownload = fetchOrDownloadAndStore(db, Module.locateFile('https://rawcdn.githack.com/grahorica/mytilda/b2ecaa20874908422d10cc61262a9e206c073be5/Utility.js')).then(addScriptToDom);
		var dataDownload =
/* // The following code would download and store the .data file as a Blob, which should be more efficient than loading an ArrayBuffer. However that seems to be buggy, so avoid it for now.
			fetchOrDownloadAndStore(db, Module.locateFile('MyTildaSite-HTML5-Shipping.data')).then(function(dataBlob) {
...
// Instead as a fallback, download as ArrayBuffer. (TODO: Figure out the bugs with the above, and switch to using that one instead)
			fetchOrDownloadAndStore(db, Module.locateFile('https://dl.dropboxusercontent.com/scl/fi/8pllckq3842ky5rdsap85/MyTildaSite-HTML5-Shipping.data?rlkey=ohsmjwzldbcw5ctn5j16rptrb&st=xu11i05v&dl=0'), 'arraybuffer').then(function(dataArrayBuffer) {
				Module['preloadedPackages'] = {};
				Module['preloadedPackages'][Module.locateFile('https://dl.dropboxusercontent.com/scl/fi/8pllckq3842ky5rdsap85/MyTildaSite-HTML5-Shipping.data?rlkey=ohsmjwzldbcw5ctn5j16rptrb&st=xu11i05v&dl=0')] = dataArrayBuffer;
				return dataJsDownload.then(addScriptToDom);
			});
